...
def phi(N, V, T, dt, state_, target_state_, control_, full_cost_grad):
    ...
    jac = jacobian(V, state_[:,:,:], control_[:,:,:], T, dt ) 
    ...
    for ind_time in range(T-1, 0, -1):
        ...
        for v_ in range(V):
            ...
            print(ind_time, phi_[0,v_,ind_time])
            
            der = 0.

            if v_ in [0,1]:
                der += full_cost_grad[0,v_,ind_time]

            for v0_ in range(V):
                der += phi_[0,v0_,ind_time] * jac[v0_,v_,ind_time-1]

            phi_[0,v_,ind_time-1] = phi_[0,v_,ind_time] - dt * der   
...
def phi1(N, V, T, n_control_vars, phi_, state_, control_, state_pre_,
    ...
    for ind_t in range(0, T):
        ...
        jac_u_ = D_u_h(V, n_control_vars, state_, control_, ind_t)
...
@numba.njit
def D_u_h(V, n_control_vars, state_, control_, t_):
...
#@numba.njit
def jacobian( V, state_, control_, T, dt ):
    
    jacobian_ = np.zeros(( V, V, T))
    C_ = np.zeros(( V,T ))
    
    for t_ in range(20,T):
        ind_var = [0,1,2,3,5,6,7,8,9,10,11,12,15,16,18,19]
        ind_var = [2,15]
        n_unknown = len(ind_var) + 1
        t_shift = np.around(len(ind_var)).astype(int)
        ...
        for j in [0]:
            ...
            for t in range(n_unknown):
                for k in range(len(ind_var)):
                    A_[t,k] = state_[0,ind_var[k],t_+t-t_shift]
                A_[t,-1] = 1.
                x_[t] = ( state_[0,j,t_+t-t_shift] - state_[0,j,t_+t-t_shift-1] ) / dt
...
def d_r_func_mu(mu, sigmarange, ds, sigma, Irange, dI, C, precalc_r, interpolate_r):
    return jac_aln.der_mu(sigma, sigmarange, ds, mu, Irange, dI, C, precalc_r)

@numba.njit
def d_r_func_sigma(mu, sigmarange, ds, sigma, Irange, dI, C, precalc_r, interpolate_r):
    return jac_aln.der_sigma(sigma, sigmarange, ds, mu, Irange, dI, C, precalc_r)

@numba.njit
def d_tau_func_mu(mu, sigmarange, ds, sigma, Irange, dI, C, precalc_tau_mu, interpolate_tau):
    return jac_aln.der_mu(sigma, sigmarange, ds, mu, Irange, dI, C, precalc_tau_mu)

@numba.njit
def d_tau_func_sigma(mu, sigmarange, ds, sigma, Irange, dI, C, precalc_tau_mu, interpolate_tau):
    return jac_aln.der_sigma(sigma, sigmarange, ds, mu, Irange, dI, C, precalc_tau_mu)

@numba.njit
def d_V_func_mu(mu, sigmarange, ds, sigma, Irange, dI, C, precalc_V, interpolate_V):
    return jac_aln.der_mu(sigma, sigmarange, ds, mu, Irange, dI, C, precalc_V)

@numba.njit
def d_V_func_sigma(mu, sigmarange, ds, sigma, Irange, dI, C, precalc_V, interpolate_V):
    return jac_aln.der_sigma(sigma, sigmarange, ds, mu, Irange, dI, C, precalc_V)