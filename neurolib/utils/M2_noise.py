...
def M2(model, noise_real, init_params, control_, target_state, c_scheme_, u_mat_, u_scheme_, max_iteration_, tolerance_, startStep_,
    ...
    state_pre_ = [ np.zeros(( N,V,max(t_pre_ndt,1) )) ] * noise_real
    ...
    for noise_ in range(noise_real):
        ...
        #print(state0_[0,0,0,::1000])
        #print(state0_[1,0,0,::1000])
        #plt.plot(model.t, model.rates_exc[0,:])
        #plt.plot(model.t, model.rates_inh[0,:])
        #plt.plot(model.t, target_state_[0,0,:])
    #plt.show()
    
    state1_ = np.zeros(( noise_real, N, V, T ))
    best_control_mean_ = np.zeros(( control_.shape ))
    best_control_ = np.zeros(( noise_real, control_.shape[0], control_.shape[1], control_.shape[2] ))

    for noise_ in range(noise_real):
        best_control_[noise_, :,:,:] = control_.copy()
        best_control_[noise_, :,:,:] = fo.setmaxcontrol(n_control_vars, best_control_[noise_,:,:,:], cntrl_max_, cntrl_min_) 
        best_control_mean_ += best_control_[noise_,:,:,:]
    ...
    while( i < max_iteration_ ):
        ...
        for noise_ in range(noise_real):
            ...
            for ind_time in range(T):
                ...
                rd_exc[0,0, ind_time] = state0_[noise_,0,0,ind_time] * 1e-3        
                rd_inh[0, ind_time] = state0_[noise_,0,1,ind_time] * 1e-3
            ...
            phi0_ = phi(N, V, T, dt, state0_[noise_,:,:,:], best_control_mean_, full_cost_grad, state_pre_[noise_], 
                    ext_exc_current,
                    ext_inh_current,
                    ext_ee_rate,
                    ext_ei_rate,
            ...
            phi1_ = phi1(N, V, T, n_control_vars, phi0_, state0_[noise_], best_control_mean_, state_pre_[noise_],
                            sigmae_ext,
                            sigmai_ext,
                            ext_ee_rate,
                            ext_ei_rate,
            ...
            grad1_ = fo.compute_gradient(N, n_control_vars, T, dt, best_control_mean_, grad_noise_[noise_], phi1_, control_variables, ie_, is_)                
            dir0_ = fo.set_direction(N, T, n_control_vars, grad0_, grad1_, dir0_, i, CGVar, tolerance_)

            if False:
                plt.plot(np.arange(75,100.1, 0.1), full_cost_grad[0,0,750:], color='blue')
                plt.plot(np.arange(75,100.1, 0.1), phi0_[0,1,750:], color='red')
                #plt.plot(np.arange(0,100.1, 0.1), grad1_[0,1,:], color='green')
                plt.show()

                plt.plot(np.arange(75,100.1, 0.1), phi0_[0,5,750:], color='blue')
                plt.plot(np.arange(75,100.1, 0.1), phi0_[0,6,750:], color='red')
                plt.plot(np.arange(75,100.1, 0.1), phi0_[0,7,750:], color='green')
                plt.plot(np.arange(75,100.1, 0.1), phi0_[0,8,750:], color='orange')
                plt.show()

                print(full_cost_grad[0,0,770:800])
                print(phi0_[0,0,770:800])
            ...
            best_control_[noise_,:,:,:] = best_control_mean_ + step_ * dir0_
            best_control_[noise_,:,:,:] = fo.setmaxcontrol(n_control_vars, best_control_[noise_,:,:,:], cntrl_max_, cntrl_min_) 
        ...
        for noise_ in range(noise_real):
            fo.set_init(model, init_vars_sim[noise_], init_vars, state_vars, startind_)
            state1_[noise_] = fo.updateFullState(model, best_control_mean_, state_vars)
    ...
    cost_node_noise = [ None ] * noise_real
    for noise_ in range(noise_real):
        cost_node_noise[noise_] = cost.cost_int_per_node(N, n_control_vars, T, dt, state1_[noise_], target_state_,
                                     best_control_mean_, 1., 1., 1., v_ = prec_variables )

    cost_node = [ np.zeros(( N, 2 )), np.zeros(( N, 6 )), np.zeros(( N, 6 )) ]

    for noise_ in range(noise_real):
        for j1 in range(2):
            cost_node[0][0,j1] += cost_node_noise[noise_][0][0,j1]
        for j1 in range(6):
            cost_node[1][0,j1] = cost_node_noise[noise_][1][0,j1]
        for j1 in range(6):
            cost_node[2][0,j1] = cost_node_noise[noise_][2][0,j1]

    for j1 in range(2):
        cost_node[0][0,j1] /= noise_real
    for j1 in range(6):
        cost_node[1][0,j1] /= noise_real
    for j1 in range(6):
        cost_node[2][0,j1] /= noise_real
    ...
    state_post_ = [ np.zeros((N,V,t_post_ndt)) ] * noise_real
    ...
    for noise_ in range(noise_real):
        fo.set_pre_post(i1, i2, bc_, bs_[noise_], best_control_mean_, state_pre_[noise_], state1_[noise_], state_post_[noise_], state_vars, model.params.a, model.params.b)
