...
def M1(model, noise_real, init_params, control_, target_state, c_scheme_, u_mat_, u_scheme_, max_iteration_, tolerance_, startStep_,
    ...
    while( i < max_iteration_ ):
        ...
        #for noise_ in range(noise_real):
            ...
            phi0_ = phi(N, V, T, dt, state0_[noise_,:,:,:], best_control_, full_cost_grad, state_pre_[noise_], 
                    ext_exc_current,
                    ext_inh_current,
                    ext_ee_rate,
                    ext_ei_rate,
            ...
            phi1_ = phi1(N, V, T, n_control_vars, phi0_, state0_[noise_], best_control_, state_pre_[noise_],
                            sigmae_ext,
                            sigmai_ext,
                            ext_ee_rate,
                            ext_ei_rate,
            ...
            #print('phi1 = ', phi1_[0,0,::500])
            #print('gradnoise ', noise_, grad_noise_[noise_][0,0,::500])

            grad_noise_[noise_] = fo.compute_gradient(N, n_control_vars, T, dt, best_control_, grad_noise_[noise_], phi1_, control_variables, ie_, is_)

            #print(noise_, grad_noise_[0][0,0,::500])
            #print(noise_, grad_noise_[1][0,0,::500])

            grad1_ += grad_noise_[noise_]

        #for noise_ in range(noise_real):
            #print(grad_noise_[noise_][0,0,1000])

        grad1_ /= noise_real

        #plt.plot(np.arange(T), grad1_[0,0,:])
        #plt.plot(np.arange(T), grad1_[0,1,:])
        #plt.show()
        ...
        for noise_ in range(noise_real):
            ...
            s_diff_ = ( np.absolute(state1_[noise_] - state0_[noise_]) < tolerance_)
            if ( s_diff_.all() ):
