...
def timeIntegration(params, control):
    ...
    seem_ext = np.zeros((N, startind + len(t)))
    seim = np.zeros((N, startind + len(t)))
    seev = np.zeros((N, startind + len(t)))
    seev_ext = np.zeros((N, startind + len(t)))
...
def timeIntegration_njit_elementwise(
    ...
    for i in range(startind, startind + len(t)):
        ...
        for no in range(N):
            ...
            mue = (Jee_max * seem[no,i-1] + Jee_max * seem_ext[no,i-1] + Jei_max * seim[no,i-1]
                   + mue_ou[no,i-1] + ext_exc_current[no, i] + control_ext[no, 0, i-startind+1]
                   )
            ...
            z1ee = cee * Ke * rd_exc[no, no]
            z1ee_ext = c_gl * Ke_gl * rowsum + c_gl * Ke_gl * ( ext_exc_rate[no, i] + control_ext[no, 2, i-startind] )
            ...
            z2ee = cee ** 2 * Ke * rd_exc[no, no]
            z2ee_ext = c_gl ** 2 * Ke_gl * rowsumsq + c_gl ** 2 * Ke_gl * ( ext_exc_rate[no, i] + control_ext[no, 2, i-startind] )
            ...
            sigmae = np.sqrt(
                ...
                + 2 * sq_Jee_max * seev_ext[no,i-1] * tau_se * taum / ((1 + z1ee_ext) * taum + tau_se)
            ...
            seem_ext_rhs = ((1 - seem_ext[no,i-1]) * z1ee_ext - seem_ext[no,i-1]) / tau_se
            ...
            seev_ext_rhs = ((1 - seem_ext[no,i-1]) ** 2 * z2ee_ext
                            + (z2ee_ext - 2 * tau_se * (z1ee_ext + 1)) * seev_ext[no,i-1]) / tau_se ** 2
            ...
            seem_ext[no,i] = seem_ext[no,i-1] + dt * seem_ext_rhs
            ...
            seev_ext[no,i] = seev_ext[no,i-1] + dt * seev_ext_rhs
            ...
            if (not filter_sigma and i == startind + len(t) - 1):
                ...
                + 2 * sq_Jee_max * seev_ext[no,i] * tau_se * taum / ((1 + z1ee_ext) * taum + tau_se)
    ...
    # z1: weighted sum of delayed rates, weights=c*K
    z1ee = cee * Ke * rd_exc[no,no]
    z1ee_ext = c_gl * Ke_gl * rowsum + c_gl * Ke_gl * ( ext_exc_rate[no,-1] + control_ext[no,2,-1] )
    # rate from other regions + exc_ext_rate
    ...
    # z2: weighted sum of delayed rates, weights=c^2*K (see thesis last ch.)
    z2ee = cee ** 2 * Ke * rd_exc[no, no]
    z2ee_ext = c_gl ** 2 * Ke_gl * rowsumsq + c_gl ** 2 * Ke_gl * ( ext_exc_rate[no,-1] + control_ext[no,2,-1] )
    ...
    sigmae = np.sqrt(
        ...
        + 2 * sq_Jee_max * seev_ext[no,-1] * tau_se * taum / ((1 + z1ee_ext) * taum + tau_se)
    ...
    return t, rates_exc, rates_inh, mufe, mufi, IA, seem, seem_ext, seim, siem, siim, seev, seev_ext, seiv, siev, siiv, mue_ou, mui_ou, sigmae_f, sigmai_f, Vmean_exc, tau_exc, tau_inh
