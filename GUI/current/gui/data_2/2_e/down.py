...
for i in i_range:
    ...
    while j < 4799:
        ...
        cost.setParams(1., factor_we, factor_ws)
        ...
        print(costnode_[i][j][0][0][0], costnode_[i][j][index_cost][0][:2])
    ...
    while not convergence_maxW[i][j]:
        ...
        if type(bestControl_maxW[i][j]) == type(None):
            ...
            weight_ = ( cost_uncontrolled[i] * (1. - t_pen[0]) / (1. - t_pen[j]) 
                       - sum(costnode_[i][j][0][0][:]) ) / sum( costnode_[i][j][index_cost][0][:] )
            ...
            weight_ = ( cost_uncontrolled[i] * (1. - t_pen[0]) / (1. - t_pen[j])
                       - sum(costnode_maxW[i][j][0][0][:]) ) / sum( costnode_maxW[i][j][index_cost][0][:] )
            cost.setParams(1., weight_ * factor_we, weight_ * factor_ws)
            weights_maxW[i][j] = cost.getParams()
            max_it = int(100)
        ...
        print('cost splitting : ', costnode_maxW[i][j][0][0][0], costnode_maxW[i][j][index_cost][0][:2])
